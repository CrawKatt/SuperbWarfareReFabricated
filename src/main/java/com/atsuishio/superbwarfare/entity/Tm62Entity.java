...
public class Tm62Entity extends Entity implements GeoEntity, OwnableEntity {
    ...
    public void touchEntity() {
        if (level() instanceof ServerLevel) {
            ...
            var entities = level().getEntities(EntityTypeTest.forClass(Entity.class), frontBox, entity -> entity != this && !(entity instanceof HangingEntity) && !(entity instanceof Display) && (entity.getBoundingBox().getSize() > 1.5 || (entity.getBoundingBox().getSize() > 0.9 && entity.getDeltaMovement().y() < -0.35))).stream().toList();

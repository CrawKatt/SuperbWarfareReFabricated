...
object EnergyAmmoStrategy : AmmoConsumeStrategy() {
    ...
    override fun consume(data: GunData, source: AmmoSource, shooter: Entity?, count: Int): Int {
        if (!isMagazine(data)) {
            // 背包型：count 就是 FE 点数
            return data.getEnergyProvider(shooter)?.let {
                EnergyStorageHelper.extract(it, count.toLong()).toInt()
            } ?: 0
        }

        val perRound = fuelPerAmmo(data)
        val extracted = data.getEnergyProvider(shooter)
            ?.let { EnergyStorageHelper.extract(it, toEnergy(data, count).toLong()).toInt() }
            ?: 0
    ...
    override fun consume(data: GunData, source: AmmoSource, handler: IItemHandler, count: Int): Int {
        ...
        val perRound = fuelPerAmmo(data)
        val extracted = EnergyStorage.ITEM.find(data.stack, null)
            ?.let { EnergyStorageHelper.extract(it, toEnergy(data, count).toLong()).toInt() }
            ?: 0
    ...
    override fun count(data: GunData, source: AmmoSource, entity: Entity?): Int {
        ...
        val energy = data.getEnergyProvider(entity)?.amount ?: 0
        return toAmmoUnits(data, energy.toInt())
    }

    override fun count(data: GunData, source: AmmoSource, handler: IItemHandler?): Int {
        if (handler == null) return 0
        val energyStorage = EnergyStorage.ITEM.find(data.stack, null) ?: return 0
        return toAmmoUnits(data, energyStorage.amount.toInt())
    }

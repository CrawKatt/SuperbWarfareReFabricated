            ...
            for (entry in converter.listMatchingResources(resourceManager).entries) {
                ...
                try {
                    entry.value.openAsReader().use { reader ->
                        ...
                        LoadingJsonCallback.EVENT.invoker().onLoading(jsonEvent)
                        ...
                        if (loaded is DefaultGunData) {
                            ...
                            LoadingDataCallback.GUN.invoker().onLoading(event)
                            if (!event.isCanceled) {
                                loaded = event.data
                            }
                        }
                        ...
                        if (loaded is DefaultVehicleData) {
                            ...
                            LoadingDataCallback.VEHICLE.invoker().onLoading(event)
                            if (!event.isCanceled) {
                                loaded = event.data
                            }
                        }

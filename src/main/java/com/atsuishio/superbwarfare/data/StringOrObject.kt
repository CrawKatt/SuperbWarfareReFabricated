...
class StringOrObject<T : Any>(@JvmField val value: T) {
    internal class StringOrObjectAdapter<T : DeserializeFromString>(type: Type, private val gson: Gson) :
        TypeAdapter<StringOrObject<T>>() {
        private val type = (type as ParameterizedType).actualTypeArguments[0]

        @Throws(IOException::class)
        override fun write(jsonWriter: JsonWriter, obj: StringOrObject<T>?) {
            if (obj == null) {
        ...
        @Throws(IOException::class)
        override fun read(jsonReader: JsonReader): StringOrObject<T> {
            when (jsonReader.peek()) {
                JsonToken.NULL -> {
                    jsonReader.nextNull()
                    return StringOrObject(gson.fromJson("{}", type))
                }
                JsonToken.BEGIN_OBJECT, JsonToken.BEGIN_ARRAY ->
                    return StringOrObject(gson.fromJson<T>(jsonReader, type))
                else -> Unit
            }

            val obj = gson.fromJson<T>("{}", type)
            obj.deserializeFromString(gson.fromJson(jsonReader, String::class.java))
            return StringOrObject(obj)
        }
    }

    internal class AdapterFactory : TypeAdapterFactory {
        override fun <T> create(gson: Gson, type: TypeToken<T>): TypeAdapter<T>? {
            if (StringOrObject::class.java.isAssignableFrom(type.rawType) && type.type is ParameterizedType) {

...
class MobGunData private constructor(
    ...
    private fun buildStack(selection: Selection): ItemStack? {
        ...
        selection.spawn.data?.let { NBTTool.saveTag(stack, TagDataParser.parseObject(it)) }
        return stack
    }
    ...
    private fun refreshGoal() {
        ...
        (mob as MobAccessor).goalSelector.addGoal(effectiveGoalPriority(), GunShootGoal(mob, this))
    }
    ...
    private fun effectiveGoalPriority(): Int {
        val conflicting = (mob as MobAccessor).goalSelector.availableGoals
    ...
    companion object {
        ...
        @JvmStatic
        fun reapplyLoadedMobs(server: MinecraftServer) {
            runCatching {
        ...
        fun removeGunGoal(mob: Mob) {
            val goalSelector = (mob as MobAccessor).goalSelector
            val goals = goalSelector.availableGoals
                .filter { it.goal is GunShootGoal<*> }
                .map { it.goal }

            goals.forEach { goalSelector.removeGoal(it) }
        }

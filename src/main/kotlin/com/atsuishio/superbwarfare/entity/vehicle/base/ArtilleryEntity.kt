...
open class ArtilleryEntity(type: EntityType<*>, world: Level) : GeoVehicleEntity(type, world) {
    ...
    override fun interact(player: Player, hand: InteractionHand): InteractionResult {
        ...
        if (player.mainHandItem.item === ModItems.FIRING_PARAMETERS && player.isShiftKeyDown) {
            setTarget(player.mainHandItem, player, "Main")
            return InteractionResult.SUCCESS
        }

        if (player.offhandItem.item === ModItems.FIRING_PARAMETERS && player.isShiftKeyDown) {
    ...
    companion object {
        ...
        val BARREL_ANIM: EntityDataAccessor<List<Int>> =
            SynchedEntityData.defineId(ArtilleryEntity::class.java, ModSerializers.INT_LIST_SERIALIZER)

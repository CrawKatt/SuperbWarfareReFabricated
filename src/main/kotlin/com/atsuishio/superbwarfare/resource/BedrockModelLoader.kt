...
object BedrockModelLoader {
    ...
    private fun onRegisterBedrockModels(event: RegisterBedrockModelEvent) {
        COMMON_MODELS.forEach {
            event.register(it, COMMON_LOADER)
        }

        COMMON_MODELS_AND_ANIMATIONS.forEach {
            event.register(it.key, COMMON_LOADER)
        }

        ARMOR_MODELS.forEach {
            event.register(it, COMMON_LOADER, ::BedrockArmorModel)
        }
    }

    private fun onRegisterBedrockAnimations(event: RegisterBedrockAnimationEvent) {
        COMMON_MODELS_AND_ANIMATIONS.forEach {
            event.register(it.value, it.key, COMMON_LOADER)
        }
    }

    private fun onModelLoaded(event: RegisterBedrockModelReloadListenerEvent) {
        event.register { models ->
            ARMOR_MODELS_POJO.clear()

            ARMOR_MODELS.forEach { path ->
                val model = models[path] as? BedrockArmorModel ?: return@forEach

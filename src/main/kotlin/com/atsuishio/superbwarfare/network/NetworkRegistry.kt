...
internal inline fun <reified T : PacketPayload> playTo(
    ...
    if (instance != null) {
        reg({ _, _ -> }, { instance }, { msg, context -> msg.handleInternal(context) })
    } else {
        reg(
            { value, buf -> encodeTo(buf, value) },
            { buf -> decodeFrom(buf) },
            { msg, context -> msg.handleInternal(context) }
        )
    }
}
...
private fun registerByteBufPayloads() {
    ...
    val handler = BiConsumer<CapabilitySyncMessage, PayloadContext> { msg, context -> msg.handleInternal(context) }

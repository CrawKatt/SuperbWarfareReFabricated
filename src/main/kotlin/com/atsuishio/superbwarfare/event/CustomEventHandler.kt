...
object CustomEventHandler {
    @JvmStatic
    fun register() {
        ReloadCallback.PRE.register(::onPreReload)
        ReloadCallback.POST.register(::onPostReload)
        ProjectileHitCallback.HIT_ENTITY.register(::onProjectileHitEntity)
        ProjectileHitCallback.HIT_BLOCK.register(::onProjectileHitBlock)
        PreKillCallback.EVENT.register { event ->
            when (event) {
                is PreKillEvent.SendKillMessage -> LivingEventHandler.onPreSendKillMessage(event)
                is PreKillEvent.Indicator -> LivingEventHandler.onPreIndicator(event)
            }
        }
    }
    ...
    fun onProjectileHitEntity(event: ProjectileHitEvent.HitEntity) {
        ...
        val data = GunData.from(stack)
        val key = BuiltInRegistries.ENTITY_TYPE.getKey(projectile.type) ?: return
    ...
    fun onProjectileHitBlock(event: ProjectileHitEvent.HitBlock) {
        ...
        if (projectile is IAdvancedHitDetection && block is TargetBlock && face != null && event.hitVec != null) {

...
open class ResearchingRecipeBuilder private constructor(
    ...
    init {
        if (this.resultItem == null && this.resultTag != null) {
            BuiltInRegistries.ITEM.getTag(this.resultTag)
                .flatMap { tag -> tag.stream().findFirst() }
                .ifPresent { holder -> this.resultItem = holder.value() }
        }
    }
    ...
    class Result(
        ...
        override fun serializeRecipeData(json: JsonObject) {
            ...
            if (base != null) json.add("base", base.toJson())
            if (addition != null) json.add("addition", addition.toJson())
            if (special != null) json.add("special", special.toJson())
            if (selectable) json.addProperty("selectable", true)
            json.addProperty("time", time)
            if (color != 0) json.addProperty("color", color)

            val result = JsonObject()
            if (resultItem != null) {
                result.addProperty("item", BuiltInRegistries.ITEM.getKey(resultItem).toString())
            } else if (resultTag != null) {
                result.addProperty("tag", resultTag.location.toString())
            }
        ...
        override fun getAdvancementId(): ResourceLocation = advancementLocation
    }
}

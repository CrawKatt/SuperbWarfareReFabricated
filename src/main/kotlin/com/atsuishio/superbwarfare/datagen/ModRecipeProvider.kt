...
class ModRecipeProvider(output: PackOutput, registries: CompletableFuture<HolderLookup.Provider>) :
    ...
    companion object {
        ...
        private fun saveControlledC4Recipe(writer: RecipeOutput) {
            val id = loc(getItemName(ModItems.C4_BOMB) + "_rc")
            val key = ResourceKey.create(Registries.RECIPE, id)

            val result = NBTTool.withTag(
                ModItems.C4_BOMB,
                2
            ) { tag -> tag.putBoolean("Control", true) }

            val advancement = writer.advancement()
                .addCriterion(
                    getHasName(ModItems.HIGH_ENERGY_EXPLOSIVES),
                    has(ModItems.HIGH_ENERGY_EXPLOSIVES)
                )
                .addCriterion("has_the_recipe", RecipeUnlockedTrigger.unlocked(key.location()))
                .rewards(AdvancementRewards.Builder.recipe(key.location()))
                .requirements(AdvancementRequirements.Strategy.OR)
                .build(ResourceLocation.fromNamespaceAndPath(id.namespace, "recipes/combat/${id.path}"))
            ...
            writer.accept(
                ...
                ShapedRecipe(
                    ...
                    ShapedRecipePattern.of(
                        mapOf(
                            'a' to Ingredient.of(ModItems.HIGH_ENERGY_EXPLOSIVES),
                            'b' to Ingredient.of(Items.COMPARATOR)
                        ),
                        "aaa",
                        "aba",
                        "aaa"
        ...
        private fun buildMiscRecipes(writer: RecipeOutput) {
            ...
            ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, ModItems.SCHEELITE, 9)
                ...
                .unlockedBy(getHasName(ModItems.RAW_SCHEELITE_BLOCK), has(commonItemTag("storage_blocks/raw_tungsten")))
        ...
        fun generatePerkResearchingRecipe(writer: RecipeOutput, type: Perk.Type, rarity: Rarity) {
            ...
            val additional = when (rarity) {
                Rarity.RARE -> ModItems.RARE_ACCESSORY_KIT
                Rarity.EPIC -> ModItems.EPIC_ACCESSORY_KIT
                ModRarities.LEGENDARY -> ModItems.LEGENDARY_ACCESSORY_KIT
                ModRarities.SUPERB -> ModItems.SUPERB_ACCESSORY_KIT
                else -> ModItems.COMMON_ACCESSORY_KIT
            }
        ...
        fun generatePerkRecycleResearchingRecipe(writer: RecipeOutput, type: Perk.Type) {
            ...
            ResearchingRecipeBuilder.item(inputPerk, 2, resTag)
                ...
                .unlockedBy("has_${resTag.location.path}", has(resTag))
                .unlockedBy(
                    getHasName(ModItems.BOOST_RESEARCH_MODULE),
                    has(ModItems.BOOST_RESEARCH_MODULE)
                )

...
class ModItemModelProvider(output: PackOutput, existingFileHelper: ExistingFileHelper) :
    ...
    override fun registerModels() {
        ...
        simpleItem(ModItems.MUZZLE_ZENIT_DTK_1)
        simpleItem(ModItems.MUZZLE_COMPENSATOR_R_17)
        ...
        simpleItem(ModItems.GRIP_RECTANGLE_VERTICAL)
    ...
    private fun simpleItem(item: RegistryObject<Item>, location: String = ""): ItemModelBuilder {
        return withExistingParent(item.id.path, ResourceLocation("item/generated"))
            .texture("layer0", loc("item/" + location + item.id.path))
    }

    private fun simpleItem(item: RegistryObject<Item>, location: String, renderType: String): ItemModelBuilder {
        return withExistingParent(item.id.path, ResourceLocation("item/generated"))
            .texture("layer0", loc("item/" + location + item.id.path)).renderType(renderType)
    }

    fun evenSimplerBlockItem(block: RegistryObject<Block>) {
        this.withExistingParent(
            Mod.MODID + ":" + ForgeRegistries.BLOCKS.getKey(block.get())!!.path,
            modLoc("block/" + ForgeRegistries.BLOCKS.getKey(block.get())!!.path)
        )
    }

    private fun gunBlueprintItem(item: RegistryObject<Item>): ItemModelBuilder {
        return withExistingParent(item.id.path, ResourceLocation("item/generated"))
            .texture("layer0", loc("item/gun_blueprint"))
    }

    private fun cannonBlueprintItem(item: RegistryObject<Item>): ItemModelBuilder {
        return withExistingParent(item.id.path, ResourceLocation("item/generated"))
            .texture("layer0", loc("item/cannon_blueprint"))
    }

    private fun handheldItem(item: RegistryObject<Item>): ItemModelBuilder {
        return withExistingParent(item.id.path, ResourceLocation("item/handheld"))
            .texture("layer0", loc("item/" + item.id.path))
    }

    private fun gunIcon(item: RegistryObject<Item>, name: String): ItemModelBuilder {
        return withExistingParent(item.id.path + "_icon", ResourceLocation("item/generated"))
            .texture("layer0", loc("item/" + name + "_icon"))
    }

    private fun gunBase(item: RegistryObject<Item>, name: String): ItemModelBuilder {
        return getBuilder(item.id.path + "_base")
            .parent(UncheckedModelFile(modLoc("displaysettings/$name.item")))
            .texture("layer0", loc("item/$name"))
    }

    private fun customSeparatedGunModel(item: RegistryObject<Item>, name: String): ItemModelBuilder {
    ...
    @JvmOverloads
    fun gunItem(item: RegistryObject<Item>, name: String = item.id.path) {

...
class ModItemTagProvider(
    ...
    override fun addTags(pProvider: HolderLookup.Provider) {
        this.tag(ModTags.Items.DUSTS)
            ...
            .addTag(commonItemTag("dusts/sulfur"))
        ...
        this.tag(ModTags.Items.INGOTS)
            ...
            .addTag(commonItemTag("ingots/uranium"))
        ...
        this.tag(ModTags.Items.STORAGE_BLOCKS)
            ...
            .addTag(commonItemTag("storage_blocks/uranium"))
            .addTag(commonItemTag("storage_blocks/sulfur"))
            .addTag(commonItemTag("storage_blocks/niter"))
            .addTag(commonItemTag("storage_blocks/raw_uranium"))
        ...
        this.tag(ModTags.Items.ORES)
            ...
            .addTag(commonItemTag("ores/uranium"))
            .addTag(commonItemTag("ores/sulfur"))
            .addTag(commonItemTag("ores/niter"))
        ...
        this.tag(ModTags.Items.RAW_MATERIALS)
            ...
            .addTag(commonItemTag("raw_materials/uranium"))
        ...
        this.tag(ModTags.Items.ORE_RATES_SINGULAR).add(
            ...
            ModItems.URANIUM_ORE, ModItems.DEEPSLATE_URANIUM_ORE,
            ModItems.SULFUR_ORE, ModItems.DEEPSLATE_SULFUR_ORE,
            ModItems.NITER_ORE, ModItems.DEEPSLATE_NITER_ORE
        )

        this.tag(ModTags.Items.ORES_IN_GROUND_STONE)
            .add(
                ModItems.GALENA_ORE, ModItems.SCHEELITE_ORE, ModItems.SILVER_ORE,
                ModItems.URANIUM_ORE, ModItems.SULFUR_ORE, ModItems.NITER_ORE
            )
        this.tag(ModTags.Items.ORES_IN_GROUND_DEEPSLATE).add(
            ...
            ModItems.DEEPSLATE_SULFUR_ORE,
            ModItems.DEEPSLATE_NITER_ORE
        )
    ...
    private fun perkItem(perk: Perk): Item {
        return ModItems.PERK_ITEMS[perk]!!
    }

    /**
     * 槽位 → 该槽位已注册的配件物品。
     *
     * tag 结构（`attachment/<桶>`、`attachment/<桶>/<稀有度>`、可研究配件的汇总）**全部由
     * [AttachmentSlots] 注册表生成**，这里只负责"哪个物品属于哪个槽位"：
     * 新增一个配件物品时把它追加到对应槽位的列表里即可，新增槽位则再加一个键。
     */
    private fun attachmentItemsBySlot(): Map<AttachmentType, List<Item>> = mapOf(
        AttachmentType.STOCK to listOf(
        ...
        AttachmentType.MAGAZINE to listOf(
        ...
        AttachmentType.BARREL to listOf(
            ...
            ModItems.MUZZLE_BREAK_BIRDCAGE,
    ...
    private fun addAttachmentTags() {
        val items = attachmentItemsBySlot()

        // 总 tag：所有槽位的并集
        this.tag(ModTags.Items.ATTACHMENT).addTags(
            *AttachmentSlots.ALL
                .mapNotNull { ModTags.Items.ATTACHMENT_BY_SLOT[it.type] }
                .toTypedArray()
        )

        for (slot in AttachmentSlots.ALL) {
            val typeTag = ModTags.Items.ATTACHMENT_BY_SLOT[slot.type] ?: continue
            val rarityTags = ModTags.Items.ATTACHMENT_RARITY_SUFFIXES
                .mapNotNull { (rarity, _) -> ModTags.Items.attachmentRarityTag(slot.type, rarity) }

            this.tag(typeTag).addTags(*rarityTags.toTypedArray())
            // 即使某个稀有度一个配件都没有，也把空 tag 声明出来，保持文件树稳定
            rarityTags.forEach { this.tag(it) }

            items[slot.type].orEmpty().forEach { addAttachmentItem(it, slot.type) }
        }

        // "可研究配件"：各槽位同稀有度的子 tag 的并集
    ...
    /** 按物品自身的稀有度把它落进 [slot] 的对应子 tag */
    private fun addAttachmentItem(item: Item, slot: AttachmentType) {
        val rarity = item.getRarity(ItemStack.EMPTY)
        val rarityTag = ModTags.Items.attachmentRarityTag(slot, rarity)
            ?: error("Unsupported attachment rarity for ${BuiltInRegistries.ITEM.getKey(item)}: $rarity")

            ...
            for (entity in uniqueEntities) {
                if (entity is VehicleEntity || entity is MissileProjectile) {
                    ...
                    if (realEntity != null) {
                        ...
                        val pt = partialTick.getGameTimeDeltaPartialTick(true).toDouble()
                        ix = Mth.lerp(pt, realEntity.xOld, realEntity.x)
                        iy = Mth.lerp(pt, realEntity.yOld, realEntity.y)
                        iz = Mth.lerp(pt, realEntity.zOld, realEntity.z)
                        renderYRot = Mth.rotLerp(pt.toFloat(), realEntity.yRotO, realEntity.yRot)
                    } else {

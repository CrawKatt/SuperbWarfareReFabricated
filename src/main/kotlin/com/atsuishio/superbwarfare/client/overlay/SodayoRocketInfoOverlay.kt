...
object SodayoRocketInfoOverlay : CommonOverlay("sodayo_rocket_info") {
    private val AP = ItemStack(ModItems.MEDIUM_ROCKET_AP)
    private val HE = ItemStack(ModItems.MEDIUM_ROCKET_HE)
    private val CM = ItemStack(ModItems.MEDIUM_ROCKET_CM)

    override fun RenderContext.render() {
        val poseStack = guiGraphics.pose()

        val reach = PlayerReachTool.getEntityReach(player)
        val lookingEntity = TraceTool.findLookingEntity(player, reach)
        ...
        for (i in lookingEntity.barrel.indices) {
            if (OBB.getLookingObb(player, reach) === lookingEntity.barrel[i]) {

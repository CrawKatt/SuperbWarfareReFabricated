...
object DroneHudOverlay : CommonOverlay("drone_hud") {
    ...
    override fun RenderContext.render() {
        ...
        if (stack.`is`(ModItems.MONITOR) && stack.getOrCreateTag().getBoolean("Using") && stack.getOrCreateTag()
                .getBoolean("Linked")
        ) {
            if (firstPerson) {
                ...
                val entity = EntityFindUtil.findDrone(player.level(), stack.getOrCreateTag().getString("LinkedDrone"))
                ...
                if (entity != null) {
                    ...
                    guiGraphics.drawString(
                        mc.font, Component.translatable("tips.superbwarfare.drone.distance")
                                + Component.literal(format1D(distance, "m")),
                        screenWidth / 2 + 10, screenHeight / 2 + 33, color, false
                    )

                    // 血量
                    guiGraphics.drawString(
                        mc.font, Component.translatable("tips.superbwarfare.drone.health") + Component.literal(
                    ...
                    for (e in entities) {
                        ...
                        RenderHelper.blit(poseStack, FRAME, x - 12, y - 12, 0f, 0f, 24f, 24f, 24f, 24f, 1f)
                        poseStack.popPose()
                    }
                }

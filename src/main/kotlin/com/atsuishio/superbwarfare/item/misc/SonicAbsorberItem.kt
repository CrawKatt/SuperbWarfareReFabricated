...
object SonicAbsorberItem : Item(Properties().rarity(Rarity.EPIC)) {
    ...
    override fun inventoryTick(
        ...
        if (parryTicks == 1) {
            ...
            stack.tag = tag
            (entity as? Player)?.cooldowns?.addCooldown(ModItems.SONIC_ABSORBER, PARRY_FAIL_COOLDOWN_TICKS)
        } else {
            (entity as? Player)?.cooldowns?.addCooldown(ModItems.SONIC_ABSORBER, PARRY_SUCCESS_COOLDOWN_TICKS)
    ...
    fun onEntityAttackedBySonicBoom(event: LivingHurtCallback.Event) {
        ...
        source.entity?.hurt(entity.damageSources().sonicBoom(entity), damage * 15)
    }
}

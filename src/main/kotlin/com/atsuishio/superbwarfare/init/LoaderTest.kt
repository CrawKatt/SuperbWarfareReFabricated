 ...
 *
 * 当前行为：
 *  1. 遍历所有 Fabric mod，读取 `META-INF/sbw/registry.json`；
 *  2. 扫描 mod 的 class roots，找出被 [TestLoaderTarget] 标注的类并加载它们；
...
object TestLoader {
    ...
    // TODO 正确在运行时读所有Rarity（需考虑EnumExtensions）
    ...
    private fun registerItems() {
        for ((namespace, info) in manifests) {
            registerItems(namespace, info.items)
            registerItems(namespace, info.guns) { GeoGunItemV2(it) }
        }
    }
    ...
    private fun registerItems(
        ...
        for ((id, item) in itemEntry) {
            ...
            Registry.register(BuiltInRegistries.ITEM, location, item.create(itemSupplier))
    ...
    private fun parseManifest(path: Path, namespace: String): LoaderInfo {
        ...
        for ((id, item) in info.items) {
            ...
            if (ResourceLocation.tryParse(id)?.path != id) {

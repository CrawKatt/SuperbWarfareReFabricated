...
object ModItems {
    ...
    ): ItemRegistry = AMMO.register(id ?: registryNameOf<T>(), ammo)

    @JvmField
    val AMMO: ItemRegister = DeferredRegister.create(ForgeRegistries.ITEMS, Mod.MODID)
    ...
    ): ItemRegistry = BLOCKS.register(id ?: registryNameOf<T>(), block)

    private fun registerBlock(block: BlockRegistry) =
        registerBlock(block.id.path) { BlockItem(block.get(), Properties()) }

    @JvmField
    val BLOCKS: ItemRegister = DeferredRegister.create(ForgeRegistries.ITEMS, Mod.MODID)
    ...
    ): ItemRegistry = VEHICLES.register(id ?: registryNameOf<T>(), item)

    // @formatter:off
    @JvmField val VEHICLES: ItemRegister = DeferredRegister.create(ForgeRegistries.ITEMS, Mod.MODID)
    ...
    @JvmField
    val PERKS: ItemRegister = DeferredRegister.create(ForgeRegistries.ITEMS, Mod.MODID)
